...
int main( int argc, char *argv[] ){
  ...
  //unsigned char tmp[TMP_BUFSIZE];
  todo_list todo;
#if 0
  unsigned char* buf = anno_secret_malloc(BUFSIZE);
  unsigned char* tmp = anno_secret_malloc(TMP_BUFSIZE);
#else
  unsigned char* buf = malloc(BUFSIZE);
  unsigned char* tmp = malloc(TMP_BUFSIZE);
#endif
  for(i=0;i<10000;i++){
  ...
  if( argc <= 1 )
    ...
    {
        int keysize;
        mbedtls_aes_context* aes=anno_secret_malloc(300);
    ...
    if( todo.chachapoly )
    {
        mbedtls_chachapoly_context* chachapoly = anno_secret_malloc(400);
