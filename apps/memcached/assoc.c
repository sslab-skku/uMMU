...
void assoc_init(const int hashtable_init) {
    ...
    primary_hashtable = x___calloc(hashsize(hashpower), sizeof(void *));
...
item *assoc_find(const char *key, const size_t nkey, const uint32_t hv) {
        ...
        (oldbucket = (hv & hashmask(hashpower - 1))) >= expand_bucket)
    {
        ...
        it = x___ld64(&primary_hashtable[hv & hashmask(hashpower)]);
    }
...
static item** _hashitem_before (const char *key, const size_t nkey, const uint32_t hv) {
    ...
    while (x___ld64(pos) && ((nkey != (x___ld64(pos))->nkey) || memcmp(key, ITEM_key(x___ld64(pos)), nkey))) {
            pos = &((x___ld64(pos)))->h_next;
    }
...
//    assert(assoc_find(ITEM_key(it), it->nkey) == 0);  /* shouldn't have duplicately named things defined */
    ...
    if (expanding &&
        (oldbucket = (hv & hashmask(hashpower - 1))) >= expand_bucket)
    {
        assert(expanding != true); // CMI
        it->h_next = old_hashtable[oldbucket];
        old_hashtable[oldbucket] = it;
    } else {
        it->h_next = x___ld64(&primary_hashtable[hv & hashmask(hashpower)]);
        x___st64(&primary_hashtable[hv & hashmask(hashpower)], it);
    }
...
void assoc_delete(const char *key, const size_t nkey, const uint32_t hv) {
    ...
    if (x___ld64(before)) {
        ...
        nxt = (x___ld64(before))->h_next;
        (x___ld64(before))->h_next = 0;   /* probably pointless, but whatever. */
        x___st64(before, nxt);
        return;
    }
...
bool assoc_iterate(void *iterp, item **it) {
    ...
    *it = NULL;
    ...
    if (iter->bucket != hashsize(hashpower)) {
        ...
        iter->it = x___ld64(&primary_hashtable[iter->bucket]);
        if (iter->it != NULL) {
